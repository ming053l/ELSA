...
class TrainableSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.backend == "elsa":
            y = twopass_attention_train(
                q,
                k,
                v,
                is_causal=self.is_causal,
                **_full_model_elsa_kwargs(q, self.elsa_kwargs),
            )
...
class TrainableTransformerBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.attn = TrainableSelfAttention(
            dim,
            heads,
            backend=backend,
            sdpa_kind=sdpa_kind,
        ...
        self.mlp = TrainableMLP(dim, hidden_dim)
...
class TrainableTransformerEncoder(nn.Module):
    """Full-model training harness using the two-pass ELSA autograd attention."""
...
def make_train_model_pair(
    *,
    ...
    is_causal: bool = False,
    elsa_kwargs: Optional[dict] = None,
) -> tuple[TrainableTransformerEncoder, TrainableTransformerEncoder]:
    baseline = TrainableTransformerEncoder(
        dim=dim,
        depth=depth,
        heads=heads,
        mlp_ratio=mlp_ratio,
    ...
    elsa = TrainableTransformerEncoder(
        dim=dim,
        depth=depth,
        heads=heads,
        mlp_ratio=mlp_ratio,
