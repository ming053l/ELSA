...
from .attention_bwd import twopass_attention_train


@dataclass(frozen=True)
class TrainPatchReport:
    vit_attention: int = 0
...
class ElsaTrainTimmVitAttention(nn.Module):
    def __init__(self, source: nn.Module, *, elsa_kwargs: Optional[dict] = None) -> None:
        ...
        # newer timm exposes attn_dim; older ViT Attention (e.g. SICNet fork) does not -> derive it.
        ...
        self.attn_drop = copy.deepcopy(source.attn_drop) if hasattr(source, "attn_drop") else nn.Dropout(0.0)
        self.elsa_kwargs = dict(elsa_kwargs or {})

    def _elsa_kwargs_for_seq(self, seq_len: int, dtype: torch.dtype) -> dict:
        kwargs = dict(self.elsa_kwargs)
        kwargs["algorithm"] = "paper_scan"
        if dtype != torch.float32 and seq_len >= 6000:
            kwargs.setdefault("block_m", 64)
            kwargs.setdefault("block_n", 256)
            kwargs.setdefault("bwd_block_m", 32)
            kwargs.setdefault("bwd_block_n", 32)
        elif dtype != torch.float32 and seq_len >= 4096:
            kwargs.setdefault("block_m", 64)
            kwargs.setdefault("block_n", 256)
            kwargs.setdefault("bwd_block_m", 32)
            kwargs.setdefault("bwd_block_n", 32)
        else:
            kwargs.setdefault("block_m", 32 if dtype == torch.float32 else 64)
            kwargs.setdefault("block_n", 128 if dtype == torch.float32 else 256)
        kwargs.setdefault("q_chunk_size", seq_len)
        kwargs.setdefault("phase1_warps", 4)
        kwargs.setdefault("phase2_warps", 4)
        kwargs.setdefault("phase1_stages", 2)
        kwargs.setdefault("phase2_stages", 1)
        return kwargs

    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        if attn_mask is not None:
            raise ValueError("ELSA ViT bwd drop-in currently supports unmasked attention only")
        if self.training and getattr(self.attn_drop, "p", 0.0):
            raise ValueError("ELSA ViT bwd drop-in does not implement attention dropout")
        batch, seq_len, _ = x.shape
        qkv = self.qkv(x).reshape(batch, seq_len, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        q, k = self.q_norm(q), self.k_norm(k)
        y = twopass_attention_train(q, k, v, **self._elsa_kwargs_for_seq(seq_len, q.dtype))
...
class ElsaTrainTimmSwinWindowAttention(nn.Module):
    def __init__(self, source: nn.Module, *, elsa_kwargs: Optional[dict] = None) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        if self.training and self.attn_drop.p:
            raise ValueError("ELSA Swin bwd drop-in does not implement attention dropout")
        ...
        y = twopass_attention_train(q, k, v, bias=bias, **self._elsa_kwargs_for_window(seq_len))
...
def _is_vit_attention(module: nn.Module) -> bool:
    # 'norm'/'attn_dim' exist only in newer timm ViT Attention; derive/Identity them in
    # the wrapper so older forks (e.g. SICNet timm) still patch (mirrors timm_dropin.py).
...
def patch_timm_attention_train(model: nn.Module, *, elsa_kwargs: Optional[dict] = None) -> TrainPatchReport:
    vit_count = 0
    swin_count = 0
    ...
    def visit(parent: nn.Module) -> None:
        ...
        for name, child in list(parent.named_children()):
            if _is_vit_attention(child):
                setattr(parent, name, ElsaTrainTimmVitAttention(child, elsa_kwargs=elsa_kwargs))
                vit_count += 1
            elif _is_swin_v1_window_attention(child):
                setattr(parent, name, ElsaTrainTimmSwinWindowAttention(child, elsa_kwargs=elsa_kwargs))
                swin_count += 1
            else:
                visit(child)
    ...
    return TrainPatchReport(vit_attention=vit_count, swin_window_attention=swin_count)
